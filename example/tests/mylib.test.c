...
static char * test_myAdd_normal() {
    ja_assert("4 + 6 == 10", myAdd(4, 6) == 10);

    return 0;
}

static char * test_myAdd_bigBrother() {
    ja_assert("2 + 2 == 5", myAdd(2, 2) == 5);

    return 0;
}

static char * test_myAdd_large() {
    ja_assert("1000 + 1000 == 2000", myAdd(1000, 1000) == 2000);

    return 0;
}

static char * test_myAdd_small() {
...
static char * test_mySub_normal() {
    ja_assert("4 - 6 == -2", mySub(4, 6) == -2);

    return 0;
}
static char * test_mySub_large() {
    ja_assert("2000 - 1000 == 1000", mySub(2000, 1000) == 1000);

    return 0;
}

static char * test_mySub_small() {
    ja_assert("0 - 0 == 0", mySub(0, 0) == 0);

    return 0;
}

static char * test_mySub_bigBrother() {
    ja_assert("2 - 1 == 1", myAdd(2, 2) == 1);
    ja_assert("2 - 2 == 0 (meant to fail)", myAdd(2, 2) == 0);
    ja_assert("2 - 3 == -1", myAdd(2, 2) == -1);
...
static char * test_myRand() {
    ja_assert("myRand() == 5", myRand() == 5);

    return 0;
}

static char * test_myRand_large() {
    int i;
    for (i = 0; i < 256; i++) {
        ja_assert("myRand() == 5", myRand() == 5);
    }
...
static char * test_myHash_normal() {
    char * word = "fish";
